...
final _txFind = RegExp(
  r'(?:^|[\s،,;:]+|(?:قال|قالت)\s*:?\s*)'
  r'و?'
  r'(?<verb>حدثنا|حدثني|حدثتنا|حدثتني|اخبرنا|اخبرني|انبانا|انباني|سمعت|عن)'
  r'(?=[\s،,;:]+|$)',
  unicode: true,
);
...
bool _isMatnNoise(String name) {
  ...
  if (RegExp(r'(?:سالتك|يزعم|ارسل|دعا|يزيدون|ينقصون|فزعمت|بشاشته|القريش|الشام|قام\s+فينا)', unicode: true)
      .hasMatch(n)) {
    return true;
  }
  if (RegExp(r'^(?:هرقل|الملك|اصحابه|ملك|فينا|رجل|امراة|قوم|ناس|الحارث\s+بن\s+هشام)$', unicode: true)
      .hasMatch(n)) {
    return true;
  }
...
bool _isStoryPerson(String foldedName) {
  return RegExp(
    r'^(?:ابا|ابو)\s+سفيان|هرقل|الحارث\b|كسري|النجاشي|المقوقس|'
    r'ابو\s+جهل|ابو\s+لهب|ابليس|شيطان|رجل$|امراة$|قوم$|ناس$',
    unicode: true,
  ).hasMatch(foldedName);
}
...
bool _isRelativeToken(String name) {
  ...
  return RegExp(
    r'^(?:ابيه|اباه|ابوه|والده|امه|امها|والدته|جده|اخيه|اخاه|عمه|خاله|مولاه|مولاها|مولاهم)$',
    unicode: true,
  ).hasMatch(f);
}
...
String _cutIsnadAr(String textAr) {
  ...
  // Plain is ى-folded to ي — use ي verb forms (اتي/صلي/نهي).
  const matnVerbs =
  ...
  for (final m in RegExp(
    r'(?:^|[\s،,;:])(?:' + matnVerbs + r')(?:\s+فينا)?\s+(?:رسول\s*الله|النبي\b|نبي\s*الله)',
    unicode: true,
  ).allMatches(plain)) {
    var pos = m.start;
    while (pos < plain.length && ' \t،,;:'.contains(plain[pos])) {
      pos++;
    }
    addCut(pos);
  }
  ...
  for (final m in RegExp(
    r'\sان\s+(?:هرقل|ابا\s+سفيان|ابو\s+سفيان|الحارث\b|كسري|النجاشي|المقوقس)'
    r'[^\n،]{0,40}?\s*[،,]?\s*' +
        storyVerb,
    unicode: true,
  ).allMatches(plain)) {
  ...
  for (final m in RegExp(
    ...
    final aheadEnd = (m.end + 60).clamp(0, plain.length);
    final ahead = plain.substring(m.end, aheadEnd);
    if (RegExp(
      r'\s*(?:(?:' +
          matnVerbs +
          r')\b|سمعت\s+رسول|رسول\s*الله|النبي\b|'
...
List<String> _extractNamesFromArIsnad(String isnad) {
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    final peel = RegExp(
      ...
      r'ان\s+(?!ه\s+سمع))',
      unicode: true,
    ).firstMatch(_foldAr(chunk));
    if (peel != null) {
      chunk = chunk.substring(0, peel.start).replaceAll(RegExp(r'^[ ،,;:]+|[ ،,;:]+$'), '');
    }
    if (chunk.isEmpty) continue;
...
List<String> extractRaviChain(String? textAr, {String? primary, String? textEn, String? textUr}) {
  ...
  var names = <String>[];
  if (ar.isNotEmpty) {
    final cut = _cutIsnadAr(ar);
    names = _extractNamesFromArIsnad(cut);
    // Append رسول الله ﷺ when Matn opens with Prophet speech/action.
    final plain = _foldAr(_stripDiac(ar));
    final cutPlain = _foldAr(_stripDiac(cut));
    var rest = '';
    if (cutPlain.isNotEmpty && plain.contains(cutPlain)) {
      rest = plain.substring(plain.indexOf(cutPlain) + cutPlain.length);
    }
    final head = rest.replaceAll(RegExp(r'^[ ،,;:]+'), '');
    final head50 = head.length > 50 ? head.substring(0, 50) : head;
    final opensWithProphet = RegExp(
    ...
    if (opensWithProphet && !names.any(_isProphetToken)) {
      names = [...names, 'رسول الله ﷺ'];
    }
  }
  ...
  if (names.isEmpty && (primary ?? '').trim().isNotEmpty) {
    ...
    if (p.isNotEmpty) names = [p];
  }
  return [for (final n in names) if (n.isNotEmpty) n];
}
